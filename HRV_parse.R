...
if (args[1] == "HRmonitor") {
    fli <- list.files(path = ".", pattern = ".*(hrv).*(csv)$")
    lenfli <- length(fli)
    
    #use only the first measurement of the day
    odate <- c(0)
    vector <- c()
    fdate <- gsub('.{9}$', '',gsub('^[a-zA-Z_]+', '',gsub('.{4}$', '',fli))) #get date
    for (i in 1:lenfli) {
        if (fdate[i]>odate) {
            vector <- c(vector,i)
            odate <- fdate[i]
    ...
    for (i in 1:lenvector) {
        ...
        while ( TRUE ) {
            ...
            strip <- c(strip,strtoi(line))
        }
        close(con)
        flitxt = paste(gsub('.{4}$', '',fli[vector[i]]),".dat")
        write.table(strip, file=flitxt,  col.names=FALSE, row.names = FALSE)
        hrv.data = CreateHRVData()
        hrv.data = SetVerbose(hrv.data,FALSE)
        hrv.data = LoadBeatRR(hrv.data, flitxt,".",scale=0.001)
        hrv.data = BuildNIHR(hrv.data)
        hrv.data = FilterNIHR(hrv.data)
        hrv.data = SetVerbose(hrv.data,TRUE)
        hrv.data = CreateTimeAnalysis(hrv.data, size=360)
        ...
        datetime <- gsub('^[a-zA-Z_]+', '',gsub('.{4}$', '',fli[vector[i]]))
